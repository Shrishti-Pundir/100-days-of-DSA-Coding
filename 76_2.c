    ...
    while (front < rear) {
        ...
        for (int i = 0; i < curr->numNeighbors; i++) {
            ...
            if (clones[neighbor->val] == NULL) {
                struct Node* newClone = (struct Node*)malloc(sizeof(struct Node));
                newClone->val = neighbor->val;
                newClone->numNeighbors = neighbor->numNeighbors;
                newClone->neighbors = (struct Node**)malloc(newClone->numNeighbors * sizeof(struct Node*));
                clones[neighbor->val] = newClone;
                queue[rear++] = neighbor;
            }
